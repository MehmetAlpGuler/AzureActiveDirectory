...
public class AzureADGraphClient {
    ...
    private String getUserMembershipsV1(String accessToken) throws IOException {
        final URL url = new URL(ServiceEndpointsConst.aadMembershipRestUri);
    ...
    public AuthenticationResult acquireTokenForGraphApi(String idToken, String tenantId)
        ...
        try {
            ...
            final AuthenticationContext context = new AuthenticationContext(
                    ServiceEndpointsConst.aadSigninUri + tenantId + "/", true, service);
            context.setCorrelationId(getCorrelationId());
            final Future<AuthenticationResult> future = context.acquireToken(ServiceEndpointsConst.aadGraphApiUri, assertion, credential, null);
            result = future.get();
        } finally {
            if (service != null) {
                service.shutdown();
